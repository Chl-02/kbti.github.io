...
function PostPage(props) {
  ...
  const onCoupangButtonClick = () => {
    const cookieAges =
      24 - new Date().getHours() <= 12
        ? 60 * 60 * (24 - new Date().getHours())
        : 60 * 60 * 12;
    _eventSenderGA('Paging', 'Click go-to-Coupang Button', 'post page');
    setIsOpened(true);
    return setCoupangCookie('coupang', true, {
      path: '/',
      maxAge: cookieAges,
      secure: true,
    });
    ...
    <div className='post-adCover-div-1'>
      <div className='post-adCover-div-2'>
        <a href={coupangLink} target='_blank' rel='noreferrer noopener'>
          <button
            className='post-result-coupang-button'
            type='primary'
            shape='round'
            style={{ width: '15rem', height: '3.5rem' }}
            onClick={onCoupangButtonClick}
          >
            쿠팡 보고 편지 {comment}
            <br />
            <p style={{ fontSize: '0.5rem', color: 'lightgray' }}>
              원치 않을 경우 뒤로 가기를 눌러주세요
            </p>
          </button>
        </a>
        <p className='post-result-coupang-comment'>
    ...
    list.map((mail, ix) =>
      mails.push(
        ...
        >
          <img
            loading='lazy'
            src={`https://web.archive.org/web/0im_/https://images.ktestone.com/PostImg/Envelop/envelop-${letterOrder[ix]}.png`}
            alt={`Envelop${ix}`}
  ...
  const nullMailRenderer = () => {
    return (
      <div className='post-page-null-maillist'>
        {lang === `Eng` ? `Nothing...` : `텅! 비었어요...`}
      </div>
    );
  };
  ...
  const metaTagRenderer = () => {
    return (
      <Helmet>
        ...
        <link
          rel='main-url'
          href='https://ktestone.com/kapable.github.io/post2021/'
        />
        ...
        <meta
          property='og:url'
          content='https://ktestone.com/kapable.github.io/post2021/'
        />
  ...
  const eachMail = (
    <div className='popup'>
      <div className='popup-inner'>
        ...
        <img
          loading='lazy'
          src={`https://web.archive.org/web/0im_/https://images.ktestone.com/PostImg/MailForm/letter-${letterOrder[mailNum]}.png`}
          alt='letter'
          className='popup-letter-form'
        />
        <h4 className='popup-head'>
          {`${userNickname}`}
          {lang === `Eng` ? '' : '에게'}
        </h4>
        ...
        <img
          loading='lazy'
          src={nickNameInput}
          className='popup-nickname-input'
          alt='popup-nickname-input'
        />
  ...
  const getList = useCallback(async () => {
    await axios
      .get(
        api_url +
          `/post?userKey=${props.match.params.username}&page=${page}&amount=10`
      )
      .then((res) => {
  ...
  async function getMail(postid) {
    await axios.get(api_url + `/post/${postid}`).then((res) => {
  ...
  if (isLogin) {
    ...
    return (
      <div className='post-page'>
        ...
        <PageBackground className='post-page-bg-div'>
          ...
          <div className='post-page-postbox-div'>
            <img
              ...
              src={postBoxImg}
              alt='POSTBOX'
              className='post-page-postbox'
            />
            <h4 className='post-page-whosname'>
              {`${userNickname}`} {lang === `Eng` ? `'s` : `님의`}
              <br></br>POST
            </h4>
            <h4 className='post-page-mailcount'>
              {mailCount}
              {lang === `Eng` ? ` letters` : `개의 편지`}
            </h4>
          </div>
          {/* Coupang or Mails */}
            ...
            <>
              <div className='post-page-btn-div'>
                <img
                  ...
                  src={sendMailImg}
                  alt='Send Mail'
                  className='post-page-send-mail-btn'
                  onClick={() => {
                    _eventSenderGA(
                      'Paging',
                      'Click Post-to-write Button',
                      'post page'
                    );
                    if (lang === `Eng`) {
                      navigate({
                ...
                />
                <CopyToClipboard
                  text={`https://ktestone.com/kapable.github.io/post2021/${props.match.params.username}/`}
                >
                  <img
                    loading='lazy'
                    src={shareMyPostImg}
                    ...
                    className='post-page-share-mypost-btn'
                  />
                </CopyToClipboard>
              </div>
        ...
        <div className='popup-div'>{ShowPopup ? eachMail : null}</div>
        {/* {AdplusAdvertiser()} */}
        <a
          target='_blank'
