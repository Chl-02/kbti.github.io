...
function PostWrite(props) {
  ...
  const getUserNickname = useCallback(async () => {
    await axios
      .get(
        api_url + `/post?userKey=${encodeURIComponent(key)}&page=1&amount=10`
      )
      .then((res) => {
    ...
    <div className='popup'>
      <div className='popup-inner'>
        <img
          ...
          src={completePopupBg}
          alt='메시지 보내기 완료!'
          className='postwrite-send-complete-postbox'
        />
        <img
          loading='lazy'
        ...
        >
          <img
            ...
            onClick={() => {
              _eventSenderGA(
                'Submitting',
                'Click write-popup-close Button',
                'post write page'
              );
              setShowPopup(false);
            }}
    ...
    <Fragment>
      <div className='write-page-div'>
        ...
        <div className='write-page-title-div'>
          <h3 className='write-page-title'>
            {lang === `Eng` ? 'Send me Your letter!' : '편지를 보내주세요!'}
          </h3>
        </div>
        <div className='write-page-postbox-div'>
          <img
            ...
            className='write-page-letter-form-img'
          />
          <h4 className='write-page-whosname'>
            {`${userNickname}`} {lang === `Eng` ? '' : '님에게'}
          </h4>
          <form className='write-page-submit-form' onSubmit={onSubmitHandler}>
            <textarea
              placeholder={
                lang === `Eng`
                  ? 'Convey your heart(under 150 characters)'
                  : '마음을 전달하세요(150자 이하)'
              }
              autoFocus
              cols={30.5}
            ...
            <img
              ...
              src={nickNameInput}
              alt='Nickname input'
              className='write-page-nickname-input-img'
            />
            <input
              placeholder={
                lang === `Eng`
                  ? 'Your nickname(under 10 characters)'
                  : '닉네임을 입력하세요(10자 이하)'
              }
              cols={20}
              rows={10}
            ...
            <button type='submit' className='write-page-submit-btn'>
              <img
                ...
                src={sendMailBtnImg}
                alt='submit letter'
                className='write-page-submit-btn-img'
              />
            </button>
          </form>
        </div>
