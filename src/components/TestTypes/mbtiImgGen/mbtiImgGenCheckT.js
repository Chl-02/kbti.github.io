...
import {
  checkOrderUserMatched,
  projectId,
  setOrderSended,
} from '../../../tools/aiImgTools';
...
const MbtiImgGenCheckT = ({ conceptType }) => {
  ...
  const [langComments, setLangComments] = useState([
    '잘못된 경로입니다.',
    '본인의 이미지만 확인할 수 있습니다.',
    '메일 확인 후 2시간 이내만 확인할 수 있습니다.',
    '로그인이 필요합니다.',
    '이미지 정보를 볼러올 수 없습니다.',
    '다운로드 중 에러가 발생했습니다',
    '전체 이미지 다운로드',
  ...
  useEffect(() => {
    if (lang === 'Eng') {
      setLangComments([
        'Wrong path',
        'You can check only your images',
        'You can check your images within 2 hours after the mail checked.',
        'LogIn needed',
        `Can't find image's paths`,
        'Error ocurred while downloading',
        'Download all images',
        'Download complete',
        'Download images',
        'Okay',
    ...
    if (lang === 'Kor') {
      return (
        <p style={{ fontSize: '0.7rem', marginBottom: '1rem' }}>
          전체 다운로드 시 <strong>압축파일 형태</strong>로 다운로드 됩니다.
          <br />
          <strong>개별 다운로드</strong>는 사진을 1초 정도 누르면 다운로드
          가능합니다
        </p>
      );
    } else if (lang === 'Eng') {
      return (
        <p style={{ fontSize: '0.7rem', marginBottom: '1rem' }}>
          It is downloaded in <strong>a compressed file format</strong> when it
          is completely downloaded.
          <br />
          You can <strong>download individually</strong> by pressing the photo
          for about 1 second
        </p>
      );
    } else if (lang === 'JP') {
      return (
        <p style={{ fontSize: '0.7rem', marginBottom: '1rem' }}>
          フルダウンロード時に<strong>圧縮ファイル形式</strong>
          でダウンロードされます。
          <br />
          <strong>個別ダウンロード</strong>
          は写真を1秒ほど押すとダウンロードできます。
        </p>
      );
    }
  }, [lang]);
  ...
  const load = useCallback(async () => {
    ...
    if (foreCoveredTheme.includes(conceptType)) {
      const presignedGetUrls = fileManagerObjects.map(async (objectName) => {
  ...
  useEffect(() => {
    if (!worktableId) {
      ...
      navigate(`/${conceptType}`);
    }
    if (cookies.get('accessToken')) {
      try {
        verifyAccessToken(cookies.get('accessToken')).then((res) => {
          checkOrderUserMatched(worktableId, res?.userId).then((resChecked) => {
            if (!resChecked.data.matched) {
              alert(langComments[1]);
              navigate(`/${conceptType}`);
            }
            if (
              dayjs().diff(dayjs(resChecked.data.imgCheckDate), 'hours') >
              imgCheckableHour
            ) {
    ...
    load();
  }, [load, navigate, worktableId, langComments, conceptType]);
  ...
  const onImagesDownload = useCallback(
    async (imgs) => {
      ...
      try {
        _eventSenderGA(
          'Downloading',
          `Click ${conceptType} Download Button`,
          'check page'
        );
    ...
    <>
      ...
      <div style={{ display: 'flex' }}>
        <div className='image-check-page-left-side-div'>
          {Array.from(
            Array(Math.ceil(images?.length / sidebarMultipleNum))
          ).map((_, index) => (
            <div
              className='image-check-page-left-side-div'
              key={`${index}-left`}
            >
              <img
                style={{ width: '2rem' }}
        ...
        {images.length > 0 ? (
          ...
          <div
            style={{ width: '100%', maxWidth: '20rem', margin: '0rem 1rem' }}
          >
            {languageIconRenderer()}
            <button
              className='image-check-page-download-button'
              onClick={() => onImagesDownload(images)}
            >
              {langComments[6]}
            </button>
            {downloadComment()}
            ...
            >
              <Progress
                type='circle'
                percent={(downloadCount / images?.length) * 100}
              />
          ...
          <div
            style={{ margin: '5rem auto', fontSize: '2rem', color: '#E9277D' }}
          >
            <LoadingOutlined />
            <p>loading...</p>
          </div>
        ...
        <div className='image-check-page-right-side-div'>
          {Array.from(
            ...
            <div
              className='image-check-page-right-side-div'
              key={`${index}-right`}
            >
              <img
                style={{ width: '2rem' }}
