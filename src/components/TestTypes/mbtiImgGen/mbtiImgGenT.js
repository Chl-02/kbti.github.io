...
const MbtiImgGen = ({ conceptType, lang }) => {
  ...
  const refundPolicy = (
    <p style={{ textAlign: 'left' }}>
      (주)쿠키로켓 (이하 회사)에서는 다음과 같은 기간 및 내용으로 환불 및
      청약철회를 보장하고 있습니다. 상품 설명에 환불 관련 안내가 기재된 경우
      다음 안내사항보다 우선 적용됩니다.
      <br />
      <br />
      1.본 상품은 회원의 구매 결제 완료 후 사진 업로드 즉시 변환이 이루어지는
      상품으로, 회원의 사진 업로드가 완료되면 전자상거래법 시행령
      제21조(소비자의 주문에 따라 개 별적으로 생산되는 재화)에 따라 청약 철회 및
      해제('청약철회 등)가 불가합니다.
      <br />
      2. 컨셉 화보는 Al에 의하여 임의로 생성되므로 회사는 컨셉 화보의 품질에
      보증할 수 없 으며 이를 이유로 한 환불도 불가합니다.
      <br />
      3. 단, 구매한 상품이 회사가 표시 또는 광고한 내용과 다르거나 계약 내용과
      다르게 이 행된 경우에는 구매한 날로부터 3개월 이내. 그 사실을 안 날 또는
      알 수 있었던 날로부 터 30일 이내에 청약철회 등을 할 수 있습니다.
      <br />
      4. 청약절회 등을 한 경우 본 상품은 즉시 회사로 반환되며, 회사는 7 영업일
      이내에 회원 이 지불한 금액을 회원에게 반환합니다.
      <br />
      5. 회사의 고의 또는 과실로 인해 회원이 구매한 유료서비스를 정상적으로
      이용할 수 없 는 등 회원에게 피해가 발생한 경우, 회사는 그에 대한 해결을
      위해서 최선의 노력을 하 며 그와 더불어 해당 피해에 대한 적절한 보상을
      합니다.
      <br />
      6. 회사는 소비자의 피해보상, 불만 처리 및 분쟁 처리에 관한 사항, 환물에
      대한 사항 등 소비자의 문제 제기에 대해서 해결하기 위해서 다음과 같은
      창구를 운영합니다.
      <br />
      카카오톡 상담(고객센터): https://pf.kakao.com/_IxnRfb/99696114
      <br />
      <br />
      유선 고객센터: 01084542518
      <br />
  ...
  const metaTagRenderer = (conceptType, lang) => {
    return (
      <Helmet>
        ...
        <meta
          property='og:url'
          content={`https://ktestone.com/${conceptType}/`}
        />
  ...
  const onGoToUploadClick = () => {
    _eventSenderGA(
      'Paging',
      `Click ${conceptType} Go-to-Upload Button`,
      'intro page'
    );
  };
    ...
    return (
      <>
        ...
        <div className='mbtiImgGen-intro-div'>
          ...
          />
          <div
            className='mbtiImgGen-intro-btn-div'
        ...
        <p style={{ color: 'grey', fontWeight: 'bolder' }}>
          *위 상품의 최대 이용 기간은 6개월입니다.
        </p>
        <p
        ...
        <Modal
          ...
          footer={[<Button onClick={handleOkOrCancel}>확인</Button>]}
        >
          {refundPolicy}
        </Modal>
      ...
      <>
        <div className='mbtiImgGen-guide-div'>
          ...
          <Link to={`/${conceptType}${lang.route}/upload`}>
            <div
              className='mbtiImgGen-guide-btn-div'
