...
const ColorPickerResult = () => {
  ...
  useEffect(() => {
    const difficult_range = tiers.find(
      (item) => item.difficulty === location.state.difficulty
    )?.range;
    setCurrentRange(difficult_range);
    const below_index = difficult_range.findIndex(
      (element) => location.state.totalTime > element.sec
    );
    if (below_index === -1) {
  ...
  useEffect(() => {
    if (!location.state.difficulty) {
      ...
      navigate('/colorPickerEasy');
    }
  }, [navigate, location]);
  useEffect(() => {
    if (currentLang === 'Eng') {
      setLangComments([
        '* Capture the record below and share it!',
        'KTEST',
        '',
        'Color',
        'Sensory',
  ...
  const onGoToIntroButtonClick = useCallback(() => {
    ...
    navigate(`/colorPicker${location.state?.difficulty}`, { state: 'again' });
    ...
    if (lang === 'Kor') {
      return (
        <Helmet>
          {/* <!-- Primary Meta Tags --> */}
          <title>
            색감 테스트 게임 | 당신의 티어는? - 케이테스트 | 타임어택
          </title>
          <meta
            name='title'
            content={`색감 테스트 게임 - 케이테스트 | 타임어택`}
          />
          <meta
            name='description'
            content={`당신의 티어는? | 타임어택`}
          ...
          <meta
            property='og:url'
            content={`https://ktestone.com/colorPickerEasy/`}
          />
          <meta
            property='og:title'
            content={`색감 테스트 게임 - 케이테스트 | 타임어택`}
      ...
      return (
        <Helmet>
          {/* <!-- Primary Meta Tags --> */}
          <title>
            Color Sensory Test Game | What is your Tier? - KTEST | Time Attack
          </title>
          <meta
            name='title'
            content={`Color Sensory Test Game  - KTEST | Time Attack`}
          />
          <meta
            name='description'
            content={`What is your Tier? | Time Attack`}
            data-react-helmet='true'
          />
          <link
            rel='main-url'
            href={`https://ktestone.com/colorPickerEasyEng/`}
          />
          ...
          <meta
            property='og:url'
            content={`https://ktestone.com/colorPickerEasyEng/`}
          />
          <meta
            property='og:title'
            content={`Color Sensory Test Game  -  KTEST | Time Attack`}
  ...
  return (
    <div>
      ...
      <p style={{ color: '#E62383', fontWeight: 'bold', fontStyle: 'italic' }}>
        {langComments[0]}
      </p>
      <img
      ...
      <div className='picker-result-info-div'>
        <p className='picker-result-info-title'>
          ...
          <span style={{ color: '#e52443', fontSize: '2rem' }}>
            {langComments[2]}
          </span>
          &nbsp;
          <span style={{ color: '#e59a23', fontSize: '2rem' }}>
            {langComments[3]}
          </span>
          &nbsp;
          <span style={{ color: '#27c6e3', fontSize: '2rem' }}>
            {langComments[4]}
          </span>
          &nbsp;
          <span style={{ color: '#6F24E5', fontSize: '2rem' }}>
            {langComments[5]}
          </span>
          &nbsp;
          <span style={{ color: '#E62383', fontSize: '2rem' }}>
            {langComments[6]}
          </span>
          &nbsp;
          <br />
          {langComments[7]}&nbsp;
          <span className='picker-result-info-highlight'>
            {location.state.difficulty.replace(currentLang, '').toUpperCase()}
          </span>
          &nbsp;{langComments[8]}
          <br />
          <span className='picker-result-info-highlight'>
            {location.state.totalTime.toFixed(1)}
            {langComments[9]}
          </span>
          {langComments[10]}
        </p>
      ...
      <OtherTestBannerRenderer
        lang='Kor'
        mainUrl={`colorPicker${location.state.difficulty}`}
      />
