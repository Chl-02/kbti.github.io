...
const ColorPicking = ({ difficulty, lang }) => {
  ...
  const [langComments, setLangComments] = useState([
    '* 시작 버튼을 누른 후 3초 뒤에 시작',
    '* 중간에 틀리면 다시 처음으로 돌아가요',
    '',
    '단계',
  ]);
  const [mode, setMode] = useState('intro');
  const [contents] = useState(
    difficulties.find((item) => item.difficulty === difficulty)
  );
  const [currentRound, setCurrentRound] = useState(1);
  const [currentRoundContent, setCurrentRoundContent] = useState(
  ...
  useEffect(() => {
    if (lang === 'Eng') {
      setLangComments([
        '* Press the Start button, start 3 seconds later',
        '* If you get it wrong in the middle, you go back to the beginning',
        'Stage',
        '',
      ]);
    }
    ...
    if (lang === 'Kor') {
      return (
        <Helmet>
          {/* <!-- Primary Meta Tags --> */}
          <title>
            색감 테스트 게임 {difficulty} | 당신의 티어는? - 케이테스트 |
            타임어택
          </title>
          <meta
            name='title'
            content={`색감 테스트 게임 ${difficulty} - 케이테스트 | 타임어택`}
          />
          <meta
            name='description'
          ...
          <link
            rel='main-url'
            href={`https://ktestone.com/colorPicker${difficulty}/`}
          />
          ...
          <meta
            property='og:url'
            content={`https://ktestone.com/colorPicker${difficulty}/`}
          />
          <meta
            property='og:title'
            content={`색감 테스트 게임 ${difficulty} - 케이테스트 | 타임어택`}
      ...
      return (
        <Helmet>
          {/* <!-- Primary Meta Tags --> */}
          <title>
            Color Sensory Test Game {difficulty} | What is your Tier? - KTEST |
            Time Attack
          </title>
          <meta
            name='title'
            content={`Color Sensory Test Game ${difficulty} - KTEST | Time Attack`}
          />
          <meta
            name='description'
  ...
  const onStartButtonClick = () => {
    _eventSenderGA(
      'Paging',
      `Click colorPicker${difficulty}Start-test Button`,
      'intro page'
    );
    setMode('start');
  };
  ...
  if (mode === 'intro') {
    ...
    return (
      <div
        ...
        style={{ padding: isLoading ? '15rem 0' : null }}
      >
        {metaTagRenderer(currentLang, difficulty)}
        {isLoading || !isReady ? null : (
          <div className='picker-header'>
            <h1>
              {langComments[2]} {currentRoundContent.round} {langComments[3]}{' '}
            </h1>
          </div>
        )}
        <PickerRenderer
        ...
        {isLoading || !isReady ? null : (
          <>
            ...
            <Progress
              ...
              percent={((currentRound - 1) / contents.rounds.length) * 100}
              showInfo={false}
            />
          </>
        )}
