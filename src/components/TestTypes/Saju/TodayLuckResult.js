...
const TodayLuckResult = (props) => {
  ...
  const onCoupangButtonClick = useCallback(() => {
    ...
    _eventSenderGA(
      'Paging',
      'Click go-to-Coupang Button(TODAY LUCK)',
      'result page'
    );
  }, [setCoupangCookie]);
  ...
  useEffect(() => {
    const getToday = async (source) => {
      try {
        await axios
          .get(saju_url + '/today/total/' + source, { withCredentials: true })
          .then((res) => {
            setResult(res?.data);
          });
      } catch (error) {
    ...
    <div className='todayLuck-result-main-div'>
      ...
      <div className='share'>
        ...
        <div className='share'>
          <CopyToClipboard
            text={
              'https://ktestone.com/kapable.github.io/todayLuck/' +
...
export default TodayLuckResult;
