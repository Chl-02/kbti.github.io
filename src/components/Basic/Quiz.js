...
class Quiz extends Component {
  ...
  renderImgAnswerOptions() {
    ...
    if (
      this.props.scoreType === 'numberScoringImg' &&
      qAndA.questionImage !== ''
    ) {
      return (
        <div>
  ...
  renderAnswerOptions() {
    ...
    while (i < qAndA.answers.length) {
      ...
      if (
        this.props.scoreType === 'numberScoring' ||
        this.props.scoreType === 'numberScoringImg'
      ) {
        _questions.push(
          <button
  ...
  pplTopbannerRenderer() {
    if (window.location.href.split('/').includes('labelStickerSKT')) {
      return (
        <div>
          <a
            ...
            target='_blank'
            href={`https://m.shop.tworld.co.kr/exhibition/view?exhibitionId=P00000330`}
          >
            <img
              loading='lazy'
      ...
      return (
        <div>
          <a
            rel='noopener noreferrer'
            target='_blank'
            href={`https://m.shop.tworld.co.kr/exhibition/view?exhibitionId=P00000346&utm_source=ktest&utm_medium=banner&utm_campaign=foldable5&referrer=`}
          >
            <img
              loading='lazy'
      ...
      window.location.href.split('/').includes('MALINGOETZ') ||
      window.location.href.split('/').includes('gardenflower') ||
      window.location.href.split('/').includes('hmall') ||
      window.location.href.split('/').includes('fatecharmtest')
    ) {
      return null;
    } else {
      return (
  ...
  render() {
    return (
      <Fragment>
        ...
        <Question
          question={this.props.qAndA[this.props.quizNum].question}
        ></Question>
        {/* In case of Quiz with Image Options */}
        <Fragment>{this.renderImgAnswerOptions()}</Fragment>
        <div className='option-btn-div'>{this.renderAnswerOptions()}</div>
        <QuestionCount
          totalCount={this.props.qAndA.length}
